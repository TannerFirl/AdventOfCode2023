...
card_strength['T'] = 10
card_strength['J'] = 1 # weakest card in part 2
card_strength['Q'] = 12
card_strength['K'] = 13
card_strength['A'] = 14
...
class Hand():
    def __init__(self, hand, bid):
        ...
        self.bid = int(bid)
        self._sorted = self._sorted.strip('J')
        num_js = 5 - len(self._sorted)
        card_count = {}
        ...
        if num_js != 0:
            if num_js == 5:
                ...
                best_j_char = sorted(card_count.keys(), key=lambda x: (card_count[x], card_strength[x]), reverse=True)[0]
                card_count[best_j_char] += num_js
